...
function mapRow(row:any) { return { ...row, images: Array.isArray(row.images) ? row.images : [], dealerVerified: Boolean(row.dealerVerified), isVerified: Boolean(row.dealerVerified) }; }
...
export async function getMarketplaceListings(input: { q?: string; make?: string; minPrice?: number; maxPrice?: number; limit?: number } = {}) {
  ...
  if(input.minPrice!=null) filters.push(gte(listing.askingPriceNgn,String(input.minPrice)));
  if(input.maxPrice!=null) filters.push(lte(listing.askingPriceNgn,String(input.maxPrice)));
