/**
 * ev-database.org Full Catalog Crawler via iRoyal Proxy
 *
 * Crawls all 1392 EVs from ev-database.org through the iRoyal Web Unlocker
 * proxy (bypasses rate-limit), parses specs, upserts into Neon DB.
 *
 * Usage: DATABASE_URL=... npx tsx scripts/batch-ev-proxy.ts [start] [end]
 *   start/end are 0-indexed offsets into the URL list.
 *   Default: 0-50 (first batch). Run in chunks to avoid proxy timeouts.
 */
...
// ── ev-database.org parser (inlined from ev-database.ts) ─────────────
...
function parseEvSpecs(html: string, url: string): Record<string, unknown> | null {
  ...
  return {
    ...
    dc_charge_port: (() => {
      const m = html.match(/Fast Charging[\s\S]*?Charge Port[\s\S]*?<td[^>]*>(.*?)<\/td>/i);
      return m?.[1]?.replace(/<[^>]+>/g, "").trim() ?? "CCS";
    })(),
    ...
    ncap_stars: (() => {
      const m = html.match(/(\d)\s*(?:out of|\/)\s*5/);
    ...
    price_eur: priceStr.includes("€") ? parsePrice(priceStr) : null,
    source_url: url,
  };
}
...
async function main() {
  ...
  const endIdx = parseInt(process.argv[3] ?? "50", 10);

  const allUrls: string[] = JSON.parse(fs.readFileSync("/tmp/ev-database-urls.json", "utf-8"));
  const urls = allUrls.slice(startIdx, endIdx);
  console.log(`=== ev-database.org Proxy Crawler ===`);
  console.log(`Batch: ${startIdx}–${endIdx - 1} of ${allUrls.length} total URLs`);
  console.log(`Fetching via iRoyal Web Unlocker proxy...\n`);
  ...
  for (let i = 0; i < urls.length; i += CONCURRENCY) {
      ...
      batch.map(async (url) => {
        try {
          ...
          if (status !== 200 || html.length < 1000) {
            return { url, error: `HTTP ${status}, len ${html.length}` };
          }
          const specs = parseEvSpecs(html, url);
          if (!specs) return { url, error: "parse failed" };
          return { url, specs };
        } catch (err) {
          return { url, error: (err as Error).message.slice(0, 100) };
        }
      })
    );
    ...
    for (const r of results) {
      if ("error" in r) {
        errors++;
        console.error(`  ✗ ${r.url.split("/").pop()}: ${r.error}`);
        continue;
      }
      try {
        const s = r.specs!;
        ...
        if (!makeInfo) {
          ...
          const [row] = await db.insert(gvoMake).values({
            categoryId: catId, name: s.brand as string, slug: slug(s.brand as string), origin: "Global",
          }).returning();
        ...
        upserted++;
        if (upserted % 10 === 0) console.log(`  ✓ ${upserted} upserted so far...`);
      } catch (err) {
        errors++;
        console.error(`  ✗ DB error ${r.specs?.brand} ${r.specs?.model}: ${(err as Error).message.slice(0, 100)}`);
      }
    }
