/**
 * Batch EV Crawl — Phase 2
 * Crawls remaining popular EVs from ev-database.org via Crawl4AI.
 * Uses direct Neon connection for one-time seeding.
 *
 * Usage: DATABASE_URL=... CRAWL4AI_API_URL=... CRAWL4AI_API_KEY=... npx tsx scripts/batch-ev-crawl-2.ts
 */
...
async function main() {
  // All EV URLs from cheatsheet
  const allUrls = readFileSync("/tmp/all_ev_urls.txt", "utf8").trim().split("\n");

  // Get existing knowledge entry source URLs to skip
  const existing = await db.select({ specs: knowledgeEntry.specs }).from(knowledgeEntry);
  const crawled = new Set(
    existing
      .map((e) => (e.specs as Record<string, unknown>)?.source_url as string)
      .filter(Boolean),
  );

  // Filter to un-crawled URLs, prioritizing popular brands
  const priorityBrands = [
    "BYD", "Tesla", "Hyundai", "Kia", "MG", "BMW", "Mercedes", "Nissan",
    "Toyota", "Honda", "Volkswagen", "Audi", "Volvo", "Renault", "Polestar",
    "Porsche", "Mini", "Lexus", "Subaru", "Peugeot", "Citroen", "Cupra",
    "Opel", "Fiat", "Skoda", "Dacia", "Smart",
  ];

  const toCrawl = allUrls
    .filter((u) => !crawled.has(u))
    .sort((a) => {
      const aMatch = priorityBrands.some((b) => a.includes(b));
      const bMatch = priorityBrands.some((b) => b.includes(b));
      return aMatch === bMatch ? 0 : aMatch ? -1 : 1;
    })
    .slice(0, 150); // crawl 150 more

  console.log(`Total URLs: ${allUrls.length}`);
  console.log(`Already crawled: ${crawled.size}`);
  console.log(`To crawl: ${toCrawl.length}`);

  // Crawl in batches of 50 via Crawl4AI
  const BATCH = 50;
  let totalUpserted = 0;
  let totalErrors = 0;
  ...
  for (let i = 0; i < toCrawl.length; i += BATCH) {
    ...
    console.log(`\n--- Batch ${Math.floor(i / BATCH) + 1}/${Math.ceil(toCrawl.length / BATCH)} (${batch.length} URLs) ---`);

    const vehicles = await crawlEvDatabase(batch);
    ...
    console.log(`  Batch done. Running total: ${totalUpserted} upserted, ${totalErrors} errors`);

    // Small delay between batches to be polite
    if (i + BATCH < toCrawl.length) {
  ...
  const trims = await db.select().from(gvoTrim);
  console.log(`\n=== Final DB State ===`);
  console.log(`Makes: ${makes.length}, Models: ${models.length}, Trims: ${trims.length}`);
  console.log(`Knowledge entries: ${totalKe.length}`);
